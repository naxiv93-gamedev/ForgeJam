...
func _update_container() -> void:
	...
	for card in _store.cards():
		...
		visual_inst.connect("need_removal", Callable(self, "_on_need_removal").bind(visual_inst))
		visual_inst.connect("clicked", Callable(self, "_on_card_clicked").bind(visual_inst))
		visual_inst.connect("focused", Callable(self, "_on_card_focused").bind(visual_inst))
...
func _clear() -> void:
	...
	for child in _cards.get_children():
		if not _store.has_card(child.instantiate().ref()):
			if _board != null:
				_board.register_last_known_transform(
					child.instantiate().ref(), _map_from(child.current_trans(true)))
