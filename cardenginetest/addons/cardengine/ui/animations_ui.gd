	...
	if seq.is_empty():
		...
		for step in seq.sequence():
			...
			if step.transi != null:
				...
				btn.connect("pressed", Callable(self, "_on_TransiBtn_pressed").bind(seq, index))
			...
			if step.val != null:
				...
				step_layout.add_child(btn)
				btn.connect("pressed", Callable(self, "_on_ValueBtn_pressed").bind(seq, index))
			...
			up_btn.connect("pressed", Callable(self, "_on_UpBtn_pressed").bind(seq, index))
			...
			down_btn.connect("pressed", Callable(self, "_on_DownBtn_pressed").bind(seq, index))
			...
			dup_btn.connect("pressed", Callable(self, "_on_DupBtn_pressed").bind(seq, step))
			...
			del_btn.connect("pressed", Callable(self, "_on_DelStepBtn_pressed").bind(seq, index))
